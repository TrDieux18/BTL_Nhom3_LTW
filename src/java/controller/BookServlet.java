...
public class BookServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // --- Xóa cookie giỏ hàng khách sạn đã đặt ---
            ...
            if (cookies != null) {
                ...
                // Nếu có đặt vé, xóa cookie vé máy bay
                // Giả sử bạn có ticketId từ request hoặc session
                String ticketId = request.getParameter("ticketId"); // hoặc lấy từ chỗ bạn lưu
                if (ticketId != null && cookie.getName().equals("cart_ticket_" + ticketId)) {
                    cookie.setMaxAge(0);
                    cookie.setPath("/");
                    response.addCookie(cookie);
                    request.getSession().setAttribute("successMessage", "Chúc mừng bạn đã đặt vé " + ticketId + " thành công.");
                        response.sendRedirect("hotelList");
                    }
                }
            }
